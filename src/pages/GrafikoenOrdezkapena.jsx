...
export default function GrafikoenOrdezkapena() {
  useDocumentTitle('Grafikoen Ordezkapena');
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 pt-20 pb-16">
        <div className="max-w-4xl mx-auto px-6">
          <Link to="/" className="text-white/70 text-sm hover:text-white mb-6 inline-block transition-colors">← tekno.eus</Link>
          <span className="text-white/70 text-sm font-medium uppercase tracking-wider block mb-3">DBH 3 · Komunikazioa</span>
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">Grafikoen Ordezkapena</h1>
        </div>
      </div>
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="mt-8 pt-8 border-t border-slate-200 flex justify-between items-center">
          ...
          <Link to="/cad-aplikazioak" className="flex items-center gap-2 text-cyan-600 font-semibold text-sm hover:text-cyan-500">
            CAD Aplikazioak <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </div>
    </div>
